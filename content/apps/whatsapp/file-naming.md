...
### Standard Android Pattern: IMG_yyyymmdd_WA####.EXT
...
Files are named according to either a UUID or SHA-1 hash that may be generated from the filename pattern documented above.

When files are transferred to the Camera Roll, the filename is changed either to:

- `IMG_####.JPG` - following the counter for other images
- `{UUID}.jpg` - from Camera Roll asset UUID
- `{First 8 of SHA-1}.jpg` - source unknown
